# encoding: utf-8

include Math

sess = "rake_task"#request.session[:session_id]
#query_number 
# all results for current query
results = Result.where(:query_number => 12, :session_id => "rake_task").order("score DESC").limit(5).all

puts "The query is #{results.first.query}"
urls=[]
# removes results with duplicate urls from the but leaves the result with the highest score 
results.delete_if do |item|
	if urls.include? item[:url]
		true
	else
		urls << item[:url]
		false
	end
end

## limit the result set to 100??
urls=nil

documents = []

file = File.open(Rails.root.join("app/assets/stopwords")).read.split

#results.first.query.split.each {|word_q| file << word_q} 

# add pluralized / singularized query terms to end of stopwords
#results.first.query.split.each {|word_q| file << word_q.pluralize; file << word_q.singularize}
file << " "

doc_collection =[]
result_phrases=[]









results.each do |result|
	
	(1..4).each do |c|
		# remove characters and stopwords from the descriptions
		result.description.downcase.gsub(/["•\[\]{}~:;|!?*+().,”“\/]/, "").gsub(/(\s-\s)/, " ").split.each_cons(c) {|phrase| result_phrases << phrase}
	end
	
	#delete if phrase starts or ends with a stopword 
	result_phrases.delete_if {|a| (file.include?(a.last) || file.include?(a.first) )}
	#result_phrases.map {|a| puts a.inspect}

	# create a hash of the results with a hash of the words and frequencies
	# {:url=> r.url, :phrases => {"carrot" => 2, "cup" => 3}}
	doc_term_freq = {:url=> result.url, :phrases => {}}
	
	# create hash of result with hashes of {term => frequency}
	result_phrases.each do |phrase|
		if doc_term_freq[:phrases].has_key? phrase
			doc_term_freq[:phrases][phrase] += 1.0
		else
			doc_term_freq[:phrases][phrase] = 1.0
		end
	end
	
	
	# add the documents with term frequencies to doc_collection array
	doc_collection << doc_term_freq
	
	result.description = result_phrases.join(" ")
	documents << result
end









# now we've got a structure that has phrase and phrase count associated with that document.
# we then get the highest frequency of any word to normalize the word frequencies i.e, (0...1)

max_frequency_phrase = 0.0
doc_collection.each do |doc_term_freq|
	doc_term_freq[:phrases].each_value {|count| max_frequency_phrase = count if count > max_frequency_phrase}
end




# here we want to get a count of the document frequency of the terms

top_doc_freq={}
doc_collection.each do |doc_term_freq|
	
	doc_term_freq[:phrases].each do |term, count| 
		# normalizing term frequency by dividing max occurance by term frequency, only (phrases > 1) # Get the "tf" in "tf-idf"
			if count > 1
				count /= max_frequency_phrase
				#puts "term #{term}, count #{count}"; puts term.inspect
				top_doc_freq.merge!({term =>0}) unless top_doc_freq.has_key? term or count < 0.2 or term.nil?
			end
	end
	
end

# accumulates the frequency of the labels occurance among the documents
top_doc_freq.each do |term, count|
	documents.each do |doc|
		if doc.description.match term.join(" ")
			top_doc_freq[term] +=1.0
		end
	end
end


num_of_docs = documents.size

# create Inverse document frequency
top_doc_freq.map do |a,b| 
	top_doc_freq[a] = log(num_of_docs/b)
end

pre_clusters = top_doc_freq.sort_by {|a,b| b}.reverse

#pre_clusters.each {|a,b| puts "#{a} #{b}"}





# getting cosine similarity between potential cluster labels

label_size = pre_clusters[0..6].size
cosine_matrix = []

#cosine_matrix.each {|array| array = Array.new(label_size)}

		label_size.times {cosine_matrix << Array.new(4, 0)}

#exit
j=0


pre_clusters[0..6].each do |label_1|
	
	#cosine_matrix << Array.new(4)
	i = 0
	pre_clusters[0..6].each do |label_2|
		
		# unique letters of this label
		if label_1 == label_2
			cosine_matrix[j][i] = "-"
			#puts "i = #{i}; j = #{j}"
			#print "-".center(19)
		else
			# this creates the frequency occurance vectors of characters
			l1_alpha = (label_1[0]+label_2[0]).join.gsub(/[^a-z0-9]/, "").scan(/./).uniq.sort.map! {|char| char = [char,0] }
			
			# Deep copy of array is required ∧this is a serialisation trick
			l2_alpha = Marshal.load( Marshal.dump(l1_alpha) )
			
			l1_chars = label_1[0].join
			l2_chars = label_2[0].join
			
			# sets label frequency occurances 
			l1_alpha.each {|freq| freq[1] = l1_chars.scan(freq[0]).count }
			l2_alpha.each {|freq| freq[1] = l2_chars.scan(freq[0]).count }
		
			dot_product =  0
			for index in 0...l1_alpha.size
				dot_product += l1_alpha[index][1] * l2_alpha[index][1]
			end
			
			mag_l1 = 0
			mag_l2 = 0
			
			
			l1_alpha.each {|count| mag_l1 += count[1]*count[1]}
			
			l2_alpha.each {|count| mag_l2 += count[1]*count[1]}
			
			mag_l1 = sqrt(mag_l1)
			mag_l2 = sqrt(mag_l2)
			
			prod_mag = mag_l1 * mag_l2
			
			similarity = dot_product / prod_mag
			
			cosine_matrix[j][i] = similarity
			
		end
		i+=1
	end
	j+=1
end	
	
#puts "cosine_matrix #{cosine_matrix}"
pre_clusters[0..6].each {|c| print c, "\t"}
puts
cosine_matrix.each do |column| 
	column.each {|ele| print ele.to_s[0..4], "\t\t\t"}
	puts
end

# cluster the labels
	
# create cluster structure that contains the cluster labels with associated results array
clusters = {}
top_doc_freq.each do |phrase|
	#puts phrase.inspect
	
	
	clusters.store(phrase, [])
end
clusters.store("other", [])
	
# go through each of the documents and add to the cluster that matches the phrase.
# if a document matches no phrase put it in the "other" cluster

documents.each do |doc|
	matched_flag = 0
	top_doc_freq.each do |phrase|
		if doc.description.match phrase[0].join(" ")
			clusters[phrase] << doc.id
			matched_flag = 1
		end
		clusters["other"] << doc.id if matched_flag ==0
	end
	
end

#puts clusters.size
# max value in matrix

#arr[arr.each_with_index.max[1]][arr[arr.each_with_index.max[1]].each_with_index.max[1]]



